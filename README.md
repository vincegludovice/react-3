...
Let's setup some default HTTP headers for axios, so we send the correct headers
with our requests.
...
Now that we have `axios` imported, we can go into the `componentDidMount` method
...
We can then update `POST DATE GOES HERE` and `POST TEXT GOES HERE` to use these
destructured values.
...
Now that we're passing down all the necessary `props` into the `Edit` component,
...
We can then update the `createPost` method in
